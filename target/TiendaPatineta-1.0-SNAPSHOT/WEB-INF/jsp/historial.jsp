<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Mis Compras - Tienda de Patinetas</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
body, .bg-light {
    background: #23272b !important;
}
.historial-container {
    background: #181c24;
    border-radius: 22px;
    box-shadow: 0 4px 32px rgba(0,255,193,0.13);
    padding: 2.5rem 2rem 2rem 2rem;
    max-width: 1100px;
    margin: 2.5rem auto 2rem auto;
    color: #fff;
}
.historial-title {
    font-size: 2.7rem;
    font-weight: 900;
    color: #0fffc1;
    margin-bottom: 2.2rem;
    display: flex;
    align-items: center;
    gap: 0.7rem;
}
.historial-table {
    width: 100%;
    border-collapse: separate;
    border-spacing: 0;
    background: #23272b;
    border-radius: 18px;
    overflow: hidden;
    box-shadow: 0 2px 16px rgba(126,48,225,0.10);
}
.historial-table th {
    background: #7e30e1;
    color: #fff;
    font-weight: 700;
    font-size: 1.1rem;
    border: none;
    padding: 1rem 0.7rem;
}
.historial-table td {
    background: #181c24;
    font-size: 1.05rem;
    border: none;
    padding: 1rem 0.7rem;
    vertical-align: middle;
    color: #fff;
}
.historial-table tr:last-child td {
    border-bottom: none;
}
.historial-table .btn {
    border-radius: 12px;
    font-size: 1.1rem;
    padding: 0.5rem 1.1rem;
    background: #0fffc1;
    color: #181c24;
    border: none;
    font-weight: 700;
    box-shadow: 0 2px 8px rgba(0,255,193,0.10);
    transition: background 0.2s, color 0.2s;
}
.historial-table .btn:hover {
    background: #7e30e1;
    color: #fff;
}
.alert-info {
    background: #23272b;
    color: #0fffc1;
    border-radius: 12px;
    border: 1.5px solid #0fffc1;
    font-size: 1.1rem;
    text-align: center;
}
@media (max-width: 900px) {
    .historial-container { padding: 1.2rem 0.2rem; }
    .historial-title { font-size: 1.5rem; }
}
    </style>
</head>
<body class="bg-light">
    <jsp:include page="includes/header.jsp" />
    <div class="container py-4 historial-container">
        <h1 class="mb-4 historial-title"><i class="fas fa-history me-2"></i>Mis Compras</h1>
        <c:choose>
            <c:when test="${empty compras}">
                <div class="alert alert-info">Aún no has realizado ninguna compra.</div>
            </c:when>
            <c:otherwise>
                <div class="table-responsive">
                    <table class="historial-table">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Fecha</th>
                                <th>Total</th>
                                <th>Acciones</th>
                            </tr>
                        </thead>
                        <tbody>
                            <c:forEach var="venta" items="${compras}">
                                <tr>
                                    <td>${venta.id}</td>
                                    <td><fmt:formatDate value="${venta.fecha}" pattern="yyyy-MM-dd HH:mm"/></td>
                                    <td>S/. ${venta.total}</td>
                                    <td>
                                        <a href="${pageContext.request.contextPath}/detalle-compra?id=${venta.id}" class="btn btn-sm btn-info">
                                            <i class="fas fa-eye"></i> Ver Detalle
                                        </a>
                                    </td>
                                </tr>
                            </c:forEach>
                        </tbody>
                    </table>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html> 